<div class="py-4 px-8 space-y-4">
    ...
    <div class="mt-4">
        ...
        @forelse ($membersList as $item => $value)
            ...
            <x-reading-card>
                ...
                @endslot
                @slot('buttons')
                    <x-action-message class="normal-case mr-1" on="saved_{{ $readingIDs[$value->member_id] }}">
                        <i class="fa-solid fa-floppy-disk font-bold text-xl text-green-800"></i>
                    </x-action-message>

                    <x-button type="button"
                        wire:click="$dispatch('openChangeMeterModal', { memberID: '{{ $value->member_id }}' })"
                        class="!bg-yellow-500 hover:!bg-yellow-600 whitespace-nowrap" wire:loading.attr="disabled">
                        <i class="fa-solid fa-tags"></i>
                        <span>&nbsp;{{ __('Change Meter') }}</span>
                    </x-button>

                    <x-button
                        wire:click="saveReading('{{ $readingIDs[$value->member_id] }}', '{{ $value->member_id }}')"
                        class="whitespace-nowrap" wire:loading.attr="disabled">
                        <i class="fa-regular fa-floppy-disk"></i>
                        <span>&nbsp;{{ __('Save') }}</span>
                    </x-button>
                @endslot
    ...
    @livewire('readings.change-meter')
</div>
