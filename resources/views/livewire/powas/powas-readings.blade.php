                            ...
                            <x-combobox class="w-full block" id="reading_date" name="reading_date"
                                ...
                                <x-slot name="options">
                                    ...
                                    {{-- <option
                                        ...
                                        {{ Carbon\Carbon::parse($readingDates[0]->reading_date)->format('F ' .
                                        $powasSettings->reading_day . ', Y') }}
                                    </option> --}}
                                    <option
                    ...
                    <x-slot name="content">
                        <div x-data="{
                                ...
                                uploadFile(event) {
                                    ...
                                    fetch('{{ route('powas.excel-upload') }}', {
                                        method: 'POST',
                                        body: formData,
                                    })
                                    .then(res => res.json())
                                    .then(data => {
                                        ...
                                        if (data.path) {
                                            $wire.call('setExcelFilePath', data.path);
                                        } else {
                            ...
                            <input id="excelFileInput" type="file" accept=".xlsx"
                                class="block mt-1 w-full border border-gray-300 rounded px-2 py-1 text-sm"
                                @change="uploadFile($event)" />
                            <p x-show="fileName" class="mt-1 text-sm text-gray-600" x-text="'Selected: ' + fileName"></p>
                            <div x-show="uploading" class="mt-2 flex items-center gap-2 text-sm text-blue-600">
    ...
    <div x-show="expanded === 'filter'" class="grid grid-cols-1 md:grid-cols-3 gap-2 overflow-hidden" x-collapse>
        ...
        <div class="w-full block mt-2 md:mt-0 gap-2">
            ...
            <div class="w-full grid grid-cols-7">
                <x-input class="w-full col-span-3" type="date" wire:model.lazy="startDate" placeholder="Start Date" />
                <div class="flex justify-center text-center items-center">
                    <x-label class="font-bold" value="{{ __('to') }}" />
                </div>
                <x-input class="w-full col-span-3" type="date" wire:model.lazy="endDate" placeholder="End Date" />
            </div>
            @error('endDate')
                            ...
                            <x-table.tbody>
                                @foreach ($powasReadings as $item)
                                    ...
                                    <x-table.tbody-tr wire:key="{{ $item->reading_id }}">
                                        ...
                                        <x-table.tbody-td class="px-2">
                                            {{ $usersList[$item->recorded_by] ?? 'N/A' }}
                                        </x-table.tbody-td>
                                    </x-table.tbody-tr>
                                @endforeach
                            </x-table.tbody>
