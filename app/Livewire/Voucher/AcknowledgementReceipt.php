...
#[Layout('layouts.app')]
class AcknowledgementReceipt extends Component
{
    public $date;
    public $recipient;
    public $amount;
    ...
    public function convertAmountToWords()
    {
        ...
        list($whole, $fraction) = explode('.', $amount);
        
        $words = $this->numberToWords((int)$whole);
        $words = ucwords($words);
        
        $t = $words . ' Pesos';
        
        if ((int)$fraction > 0) {
            $fractionWords = $this->numberToWords((int)$fraction);
            $fractionWords = ucwords($fractionWords);
            $t .= ' and ' . $fractionWords . ' Centavos';
        } else {
            $t .= ' Only';
        }

        $this->amountInWords = $t;
    }
    ...
    {
        return view('livewire.voucher.acknowledgement-receipt');
    }
}
