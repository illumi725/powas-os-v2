...
class Fis extends Component
{
    ...
    public function autoBalanceFS()
    {
        // 1. Authorization Check
        if (!Auth::user()->hasRole('treasurer') && !Auth::user()->hasRole('admin')) {
             $this->dispatch('alert', [
                'message' => 'Unauthorized action. Only Treasurers can auto-balance.',
                'type' => 'error'
            ]);
            return;
        }
        ...
        $this->fetchData(); // Ensure fresh data
        ...
        // Calculate Assets
        foreach ($this->chartOfAccount as $acc) {
            
            // Only sum up Assets
            if ($acc->account_type == 'ASSET') {
                 $totalAssets += $this->newBeginningBalances[$acc->account_number];
            }
            elseif ($acc->account_type == 'LIABILITY') {
                 $totalLiabilities += $this->newBeginningBalances[$acc->account_number];
            }
            elseif ($acc->account_type == 'EQUITY') {
                 $totalEquity += $this->newBeginningBalances[$acc->account_number];
            }
        }
        ...
        foreach ($this->chartOfAccount as $acc) {
             if ($acc->account_type == 'REVENUE') {
                 ...
                 $netIncome += $this->newBeginningBalances[$acc->account_number];
        ...
        // Blade adds Net Income to total Equity.
        $trueEquity = $totalEquity + $netIncome;

        // Discrepancy = (L + E) - A
        $discrepancy = ($totalLiabilities + $trueEquity) - $totalAssets;
        $discrepancy = round($discrepancy, 2);

        if ($discrepancy == 0) {
             $this->dispatch('alert', [
                'message' => 'Financial Statement is already balanced.',
                'type' => 'success'
            ]);
            return;
        }
        ...
        try {
            ...
            if ($discrepancy > 0) {
                // (L+E) > A. Need to reduce E.
                // Debit Expense (Inventory Adjustment / Miscellaneous Expense)
                $targetAcc = ChartOfAccounts::where('account_number', '510')->first(); // Misc Expense
                $accountNumber = '510';
                $side = 'DEBIT';
            } else {
                // (L+E) < A. Need to increase E.
                // Credit Revenue (Misc Income)
                $targetAcc = ChartOfAccounts::where('account_number', '406')->first(); // Misc Income
                $accountNumber = '406';
                $side = 'CREDIT';
            }
            
            if (!$targetAcc) {
                throw new \Exception("Adjustment account ($accountNumber) not found in Chart of Accounts.");
            }

            $currentDate = Carbon::now()->format('Y-m-d');
            $journalEntryNumber = CustomNumberFactory::journalEntryNumber($this->powasID, $currentDate);
            $amount = abs($discrepancy);

            Transactions::create([
                'trxn_id' => CustomNumberFactory::getRandomID(),
                'account_number' => $accountNumber,
                'description' => 'System Auto-Balance: Financial Statement Adjustment',
            ...
            ActionLogger::dispatch(
                'create', 
                "Auto-balanced FS. Adjustment: $amount to $side $accountNumber",
                Auth::user()->user_id,
                'transactions',
                $this->powasID
            );
            ...
            $this->dispatch('alert', [
                'message' => "Financial Statement Balanced! Adjusted by " . number_format($amount, 2),
                'type' => 'success'
            ]);
            ...
            DB::rollBack();
            $this->dispatch('alert', [
                'message' => 'Error balancing FS: ' . $e->getMessage(),
                'type' => 'error'
            ]);
        }
    }
